...
function lockKeyFor(batchId: string, locationId: string): string {
  return `stock:${batchId}:${locationId}`;
}

/**
 * The authoritative stock mutation logic. Runs inside the caller's
 * transaction (`tx`) — never opens its own.
 *
 * Every movement:
 *   1. Serialises concurrent movements for the same (batch, location) with a
 ...
 *      The lock is an XACT lock: it is held until the surrounding transaction
 *      commits/rolls back, which is exactly what we want when composing
 *      TRANSFER_OUT + TRANSFER_IN + status update in one transaction.
 *   2. Re-validates the batch/product/location inside the transaction.
 *   3. Computes the new balance, guards against negative stock for OUT
 *      movements, updates InventoryStock and inserts the immutable
 *      StockTransaction row together (all-or-nothing).
 */
export async function recordMovementInTransaction(
  ...
  const lockKey = lockKeyFor(input.batchId, input.locationId);
...
/**
 * THE only approved way to change inventory quantities for standalone
 * operations (opening stock, adjustments, expiry actions, ...).
 *
 * Creates its own transaction and delegates to
 * `recordMovementInTransaction`, so the business logic lives in exactly one
 * place. Composed flows (e.g. transfer completion) call
 * `recordMovementInTransaction` directly inside their own transaction.
 */
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
      try {
        ...
        return await prisma.$transaction(
          (tx) => recordMovementInTransaction(tx, input),
          {
            maxWait: 10_000,
            timeout: 30_000,
          },
          ...
          logger.warn(
            {
              err: error,
              attempt,
              lockKey: lockKeyFor(input.batchId, input.locationId),
            },
